...
//#define IMGUI_IMPL_OPENGL_ES1 1
...
static cocos2d::EventListenerTouchOneByOne* g_EventListenerTouch;
...
bool ImGui_ImplCocos2dx_Init(bool install_callbacks)
{
    ...
    std::string strGlslVersion = static_cast<std::string>(reinterpret_cast<const char *>(glslVersion));
    float fGlslVersion = atof(strGlslVersion.c_str());
    if(fGlslVersion <= 1.0f)
        glsl_version = "#version 100";
    else if(fGlslVersion < 3.0f)
...
//#endif
...
#else
    g_EventListenerTouch = cocos2d::EventListenerTouchOneByOne::create();
    g_EventListenerTouch->onTouchBegan = [](cocos2d::Touch* touch, cocos2d::Event*)
    {
    ...
    g_EventListenerTouch->onTouchMoved = [](cocos2d::Touch* touch, cocos2d::Event*)
    {
        const auto loc = touch->getLocationInView();
        g_CursorPos.x = loc.x;
        g_CursorPos.y = loc.y;
    ...
    g_EventListenerTouch->onTouchEnded = [](cocos2d::Touch* touch, cocos2d::Event*)
    {
        const auto loc = touch->getLocationInView();
        g_CursorPos.x = loc.x;
        g_CursorPos.y = loc.y;
    ...
    g_EventListenerTouch->onTouchCancelled = [&](cocos2d::Touch* touch, cocos2d::Event*)
    {
        g_CursorPos = ImVec2(-FLT_MAX, -FLT_MAX);
        g_MouseJustPressed[0] = false;
    };
    cocos2d::Director::getInstance()->getEventDispatcher()->addEventListenerWithFixedPriority(g_EventListenerTouch, -1);
...
void ImGui_ImplCocos2dx_NewFrame()
{
    ...
    io.DeltaTime = (io.DeltaTime == 0.0f ? 0.0001f : io.DeltaTime);
    ...
    // Update Swallow Touch
    bool isHovered = ImGui::IsWindowHovered(ImGuiHoveredFlags_AnyWindow);
    g_EventListenerTouch->setSwallowTouches(isHovered);
#endif
}
